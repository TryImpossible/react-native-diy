...
def auto_install(apk_path):
    cmd = 'adb install -r ' + apk_path
    try:
        if len(os.popen('adb devices').readlines()) == 3:
            result = os.popen(cmd).readlines()
            print 'install result: ', result[0]
        else:
            print 'please check your adb devices'
    except Exception:
        print 'install fail'
...
def main():
    ...
    build_type = raw_input('请选择构建类型[dev，alpha，beta，rc]，默认dev：')
    if not build_type:
        build_type = 'dev'
    ...
    version_name = raw_input('请输入版本名称，默认值%s：' % default_version_name)
    if not version_name:
        version_name = default_version_name
    # 版本号，Numbers，默认最近版本号
    version_code = raw_input('请输入版本号，默认值%s：' % defalut_version_code)
    if not version_code:
    ...
    is_upload_fir = raw_input('是否上传至fir，是请输入true，否请输入false，默认false：')
    if not is_upload_fir or str(is_upload_fir).lower() == 'false':
        is_upload_fir = False
    if is_upload_pgyer or is_upload_fir:
        # 版本更新说明，String，默认空
        update_description = raw_input('请输入更新说明，默认空：')
        # 是否邮件通知， Boolean true false，默认false
        is_mail_notification = raw_input(
            '是否使用邮件通知，是请输入true，否请输入false，默认false：')
        ...
        if is_mail_notification:
            ...
            mail_receivers = raw_input(
                '若使用邮件通知，请输入邮件接受者，以英文逗号分隔，例123@qq.com,456@sina.cn：')
            if not mail_receivers:
                mail_receivers = raw_input('请输入格式正确的邮箱：')
     ...
     # 是否自动安装，Boolean true false，dev构建环境下默认true，其它默认false
    is_auto_install = raw_input(
        '是否自动安装，Boolean true false，dev构建环境下默认true，其它默认false：')
...
# example: python cli/builder/setup.py
